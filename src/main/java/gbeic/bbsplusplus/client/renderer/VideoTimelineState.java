 ...
 *
 * 它记录当前正在渲染的回放 tick，并汇总不同时间轴控件的播放头拖动状态。
 * 视频渲染器因此可以使用真实影片时间倒帧，并在拖动期间保持上一帧而不反复寻帧。
 */
public final class VideoTimelineState
{
    ...
    public static void beginFilmRender(int tick)
    {
        renderTick = tick;
        renderingFilm = true;
    }
    ...
    public static int getFilmTick(int fallback)
    {
