...
| 反转时间线滚动         | 关   | 开启后，在影片编辑器按住 Ctrl 滚动鼠标滚轮控制时间线时，反转时间线前进/后退的方向                                 |
...
| 撤销历史步数           | 50   | 设置撤销/重做历史最多保留多少步操作。超过后会自动丢弃最旧记录，避免历史列表无限堆积                               |
...
- 新增指令：`/bbsplusplus trigger <x> <y> <z> <触发器ID>`
  - 示例：`/bbsplusplus trigger 100 64 200 0`（触发指定坐标模型方块粒子的「触发器 0」）
- 已知限制与说明：
